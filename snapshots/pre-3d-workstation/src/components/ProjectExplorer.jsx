...
export function ProjectExplorer({ reduced }) {
  ...
  const arrive = useCallback(() => setCharged(active), [active])
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const cards = [...container.querySelectorAll('.output-position')]
    ...
    container.querySelectorAll('.output-position').forEach(el => observer.observe(el))
  ...
  return <section id="work" className="work section-shell" ref={root}>
    ...
    <div className="output-system" ref={scene}>
      <svg className="output-branches" viewBox={`0 0 ${routes.width} ${routes.height}`} width={routes.width} height={routes.height} aria-hidden="true">
        ...
        {active !== null && routes.legs[active] && <FlowingPulse key={`pulse-${active}`} d={routes.legs[active]} reduced={reduced} duration={0.65} scope=".output-system" color={folderColors[active]} repeat={false} onArrival={arrive} />}
    ...
    <div className="explore-caption mono"><span><i className="status-dot" /> Follow your curiosity.</span><span>Select a card to open its projects ↗</span></div>
    <dialog id="project-folder" ref={panel} className="project-folder folder-dialog" aria-labelledby="folder-title" style={{ '--folder-accent': folderColors[opened ?? 0] }} data-lenis-prevent onCancel={closeCategory} onClose={closeCategory} onClick={event => {
      ...
      <div className="folder-projects">{projects.filter(p => p.category === current.id).map((project, index) => <article className="project-entry" key={project.id}>
