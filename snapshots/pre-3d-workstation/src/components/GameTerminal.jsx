...
import { BezelTrail } from './BezelTrail'
import { ArcadeDesk } from './ArcadeDesk'
...
export function GameTerminal({ reduced }) {
  ...
  return <section id="quintex" aria-label="Hidden arcade" onPointerEnter={() => { if (reduced) setPowered(true) }} onFocus={() => { if (reduced) setPowered(true) }} className={`game-section section-shell secret-arcade ${ready ? 'terminal-powered' : ''} ${screenOn ? 'screen-ready' : ''}`} ref={root}>
    <div className="game-station" data-state={state}>
      <ArcadeDesk />
      <div className="monitor"><BezelTrail powered={powered} reduced={reduced} visible={inView} /><div className="monitor-screen">
      ...
      <div className="console-stage">
        ...
        <button className={`cartridge secret-cartridge ${state !== 'standby' ? 'inserted' : ''}`} aria-disabled={!screenOn} disabled={state !== 'standby' && state !== 'running'} onClick={() => { if (screenOn) setState(state === 'running' ? 'standby' : 'booting') }} aria-label={state === 'running' ? 'Eject cartridge' : 'Insert cartridge and start game'} title={state === 'running' ? 'Eject cartridge' : 'Insert cartridge'}><span className="cartridge-ridges" /><span className="cartridge-label"><span className="cartridge-cover" role="img" aria-label="Quintex fantasy village cover art" /><span className="cartridge-title">Quintex RPG</span></span><span className="cartridge-pins" /></button>
