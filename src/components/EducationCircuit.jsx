...
export function EducationCircuit({ reduced }) {
  ...
  useEffect(() => {
    ...
    else pairs.flat().forEach(card => card.classList.add('is-revealed'))
    return () => { resize.disconnect(); intersection.disconnect(); cancelAnimationFrame(frame); delete section.dataset.educationPower; delete section.dataset.educationReveal; pairs.flat().forEach(card => card.classList.remove('is-revealed')) }
  }, [reduced])
